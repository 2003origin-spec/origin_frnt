...
export default function DoubtSolver({ onBack, user }: DoubtSolverProps) {
  ...
  // Message queue: messages sent while a reply is in-flight are held here
  // and drained one-at-a-time after each reply arrives.
  const queuedMessagesRef = useRef<string[]>([]);
  const [queuedCount, setQueuedCount] = useState(0);
  const isBusyRef = useRef(false); // sync mirror of isTyping to avoid stale closures
  ...
  const handleSendMessage = async (overrideText?: string) => {
    ...
    // While a reply is in-flight, queue extra messages instead of firing them immediately.
    // overrideText is only set when draining from the queue, so skip the queue check then.
    if ((isBusyRef.current || isTyping || isProcessingImage) && overrideText === undefined) {
      queuedMessagesRef.current.push(outboundMessage);
    ...
    try {
      ...
      // Drain one queued message after this reply lands
      const next = queuedMessagesRef.current.shift();
      if (next !== undefined) {
        setQueuedCount(c => Math.max(0, c - 1));
        setTimeout(() => void handleSendMessage(next), 0);
      }
    }
  };
        ...
        <div className="flex items-center gap-3 sm:gap-4 min-w-0">
          ...
          <div className="relative">
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full neu-inset flex items-center justify-center overflow-hidden">
              <img
                src={(() => {
                  const sub = activeSession?.subject ?? selectedSubject;
                  if (sub === 'phy') return '/ori2d/ori-physics.png';
                  if (sub === 'chem') return '/ori2d/ori-chemistry.png';
                  if (sub === 'math') return '/ori2d/ori-maths.png';
                  if (sub === 'bio') return '/ori2d/ori-biology.png';
                  return '/ori2d/ori-happy.png';
                })()}
                alt="Ori"
          ...
          <div className="flex flex-col min-w-0">
            {viewMode === 'chat' && activeSession && isEditingTitle ? (
              ...
              >
                <h1 className="text-sm sm:text-xl font-bold text-foreground tracking-wide leading-none truncate max-w-[200px] md:max-w-none">
          ...
          <div className="flex-1 flex overflow-hidden relative">
              ...
              {isSidebarOpen && (
                <>
                  ...
                  <motion.aside
                    ...
                    animate={{ x: 0, opacity: 1, width: 190 }}
                    exit={{ x: -190, opacity: 0 }}
                  ...
                  <div className="p-2 flex flex-col h-full">
                    <div className="flex items-center gap-2 mb-2 px-1">
                      <h3 className="text-[9px] font-bold text-muted-foreground uppercase tracking-[0.2em] flex-1">Sessions</h3>
                      <button
                        onClick={() => setIsSidebarOpen(false)}
                        className="p-1 rounded-lg neu-raised text-muted-foreground hover:text-foreground transition-all outline-none"
                        title="Hide Sidebar"
                      >
                      ...
                      <button
                        ...
                        className="p-1 rounded-lg neu-raised text-primary hover:text-primary/80 transition-all"
                    ...
                    <div className="space-y-1 overflow-y-auto pr-1 custom-scrollbar flex-1">
                      {sessions.map(s => (
                        <div
                          ...
                          className={`w-full p-2 rounded-lg transition-all text-left group cursor-pointer ${activeSession?.id === s.id ? 'neu-inset border border-primary/25' : 'neu-raised hover:border-primary/20'}`}
                          role="button"
                          tabIndex={0}
                        >
                          {editingSidebarId === s.id ? (
                            ...
                            <div className="flex items-start gap-2 justify-between min-w-0">
                              <div className="min-w-0 flex-1">
                                <p className={`text-xs font-bold truncate ${activeSession?.id === s.id ? 'text-primary' : 'text-muted-foreground group-hover:text-foreground'}`}>{s.title}</p>
                                <p className="text-[9px] text-muted-foreground/50 mt-0.5">{new Date(s.updatedAt || s.createdAt).toLocaleDateString()}</p>
                              </div>
                              <button
                                type="button"
                                onClick={(e) => {
            ...
            {activeSession && (
              <section className="flex-1 flex flex-col h-full overflow-hidden bg-transparent">
                ...
                <div className="chat-canvas relative flex-1 overflow-y-auto px-4 sm:px-6 py-4 sm:py-6 custom-scrollbar">
                  <ChatBackdrop />
                ...
                <div className="p-3 border-t border-border/30 bg-background">
                  <div className="max-w-5xl mx-auto">
                    ...
                    <AnimatePresence>
                      {queuedCount > 0 && (
                        <motion.div
                          ...
                          className="mb-2 flex items-center gap-2 rounded-xl border border-amber-500/20 bg-amber-500/8 px-3 py-1.5 text-[11px] text-amber-500/80 font-medium"
                        >
                          <div className="w-1.5 h-1.5 rounded-full bg-amber-500/60 animate-pulse shrink-0" />
                          {queuedCount === 1 ? '1 message queued' : `${queuedCount} messages queued`} — will send after current reply
                        </motion.div>
                      )}
                    </AnimatePresence>
                    {highlightedText ? (
                    ...
                    <div className={`neu-raised ${isRecording ? 'ring-1 ring-red-500/40' : ''} rounded-xl sm:rounded-[28px] p-1 sm:p-2 flex items-end gap-1 sm:gap-2 transition-all`}>
                      ...
                      >
                        {(isTyping || isProcessingImage) && !isRecording
                          ? <Loader2 className="w-5 h-5 animate-spin" />
                          : isRecording ? <div className="w-5 h-5 flex items-center justify-center font-bold">●</div>
                          : <Send className="w-5 h-5" />}
                      </button>
                    </div>
                    <div className="flex items-center justify-center gap-2 mt-3">
                      <Image src="/O3-Origin-Logo.png" alt="O3 Origin" width={60} height={16} className="h-4 w-auto grayscale opacity-50" />
            ...
            {quickTopics.map((topic) => (
              ...
              >
                <div className="neu-inset flex h-14 w-14 items-center justify-center rounded-xl transition-transform group-hover:scale-105 overflow-hidden">
                  {topic.oriImg ? (
                    <Image src={topic.oriImg} alt={topic.name} width={48} height={48} draggable={false} className="object-contain select-none" />
                  ) : (
                    ...
                    <div className="flex min-w-0 flex-1 items-center gap-3">
                      <div className="neu-inset flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl overflow-hidden">
                        {s.subject && SUBJECT_ORI[s.subject] ? (
                          <Image src={SUBJECT_ORI[s.subject]} alt={s.subject} width={32} height={32} draggable={false} className="object-contain select-none" />
                        ) : (
      ...
      <div className={`flex gap-4 max-w-[85%] ${isAI ? 'flex-row' : 'flex-row-reverse'}`}>
        {isAI && (
          <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl sm:rounded-2xl bg-primary/15 border border-primary/30 flex items-center justify-center flex-shrink-0 mt-1 shadow-lg shadow-primary/10 overflow-hidden">
            <Image src="/ori2d/ori-happy.png" alt="Ori" width={40} height={40} className="object-contain" />
          </div>
        )}
        <div className={`p-3 sm:p-5 rounded-xl sm:rounded-[28px] text-xs sm:text-[15px] leading-relaxed relative ${isAI
          ...
          {message.image && (
            <div className="mb-4 rounded-2xl overflow-hidden border border-white/10 max-w-[200px]" style={{ aspectRatio: '4/3' }}>
              <img src={message.image} alt="Uploaded problem" className="w-full h-full object-cover" />
            </div>
          )}
      ...
      <div className="max-w-5xl mx-auto flex flex-wrap gap-1.5">
        {pills.map((pill) => {
          ...
          return (
            ...
            >
              <Icon className={cn('w-3 h-3', isActive ? pill.color : 'opacity-60')} />
              {pill.name}
            </button>
          );
        })}
  ...
  return (
    <div className="flex justify-start gap-4">
      <div className="w-10 h-10 rounded-2xl bg-primary/15 border border-primary/30 flex items-center justify-center overflow-hidden">
        <Image src="/ori2d/ori-thinking.png" alt="Ori Thinking" width={40} height={40} className="object-contain animate-pulse" />
      </div>
