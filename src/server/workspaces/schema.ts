/**
 * Idempotent runtime ensure for the Phase 1 workspace schema.
 * The canonical SQL lives in src/db/migrations/20260520_phase1_workspaces.sql;
 * this helper mirrors it so dev/test environments without a migration runner
 * still get the tables on first use.
 */
...
declare global {
  var __originWorkspaceSchemaEnsured: boolean | undefined;
  var __originWorkspaceSchemaPromise: Promise<void> | undefined;
}

const MIGRATION_ID = "20260520_phase1_workspaces";
...
async function applyMigrationRecord(client: PoolClient): Promise<void> {
  await client.query(
    "INSERT INTO app.migrations (id, name) VALUES ($1, $2) ON CONFLICT (id) DO NOTHING",
    [MIGRATION_ID, "phase 1 workspace foundation + rbac"],
  );
}

export async function ensureWorkspaceSchema(): Promise<void> {
  if (!isUserPostgresConfigured()) return;
  if (globalThis.__originWorkspaceSchemaEnsured) return;
  if (!globalThis.__originWorkspaceSchemaPromise) {
    globalThis.__originWorkspaceSchemaPromise = (async () => {
      ...
      globalThis.__originWorkspaceSchemaPromise = undefined;
      throw error;
    });
  }
  await globalThis.__originWorkspaceSchemaPromise;
}
