...
/**
 * POST /api/internal/cron/crystallize
 *
 * Proxy for Vercel Cron to trigger the Patent C Knowledge Crystallization
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Using batch_size=25 as per the deployment plan
    const response = await fetch(`${serviceUrl}/api/v1/internal/crystallize?batch_size=25`, {
      method: "POST",
      headers: {
