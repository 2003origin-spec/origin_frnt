...
const StatCard = ({ title, value, change, isPositive, icon: Icon, color }: any) => (
    <div className="neu-raised rounded-3xl p-8 hover:opacity-90 transition-all group relative overflow-hidden">
        ...
        <div className="mt-4 flex items-center gap-2">
            ...
            <span className="text-[10px] font-bold text-muted-foreground uppercase">Live Metrics</span>
        </div>
    </div>
);
        ...
        <AdminLayout>
            <div className="space-y-12 pb-24">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 neu-raised rounded-[2.5rem] p-10 flex flex-col">
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
                            <div>
                                <h3 className="text-2xl font-black uppercase tracking-tight text-foreground mb-2">Revenue <span className="text-muted-foreground font-normal italic">Dynamics</span></h3>
                                <p className="text-xs text-muted-foreground font-bold uppercase tracking-widest leading-relaxed">Year-to-date subscription growth and retention volatility.</p>
                            </div>
                            <div className="flex items-center gap-3">
                                <button className="p-3 bg-card/40 border border-border/30 rounded-2xl text-muted-foreground hover:text-foreground transition-all"><Download className="w-5 h-5" /></button>
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="neu-raised rounded-[2rem] p-8">
                            <div className="flex items-center justify-between mb-8">
                                <h3 className="text-sm font-black uppercase tracking-[0.22em] text-muted-foreground">Coupon Engine</h3>
                            ...
                            <div className="space-y-4">
                                {[
                                    ...
                                    <div key={i} className="flex items-center justify-between p-4 bg-card rounded-2xl border border-border/30">
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 rounded-xl bg-muted/40 flex items-center justify-center">
                                                <Ticket className="w-5 h-5 text-muted-foreground" />
                                            </div>
                                            <div>
                                                <p className="text-xs font-black text-foreground">{promo.code}</p>
                                                <p className="text-[10px] font-bold text-muted-foreground uppercase">{promo.discount} OFF</p>
                                            </div>
                                        </div>
                        ...
                        <div className="bg-card border border-border/30 rounded-[2rem] p-8 overflow-hidden relative">
                            <div className="absolute top-0 right-0 p-4 opacity-10">
                                <Terminal className="w-12 h-12" />
                            </div>
                            <h3 className="text-sm font-black uppercase tracking-[0.22em] text-muted-foreground mb-6">Recent Ledger</h3>
                            <div className="space-y-4 font-mono">
                                {[
                                    ...
                                    <div key={i} className="flex items-start justify-between gap-4">
                                        <span className="text-[10px] text-muted-foreground truncate leading-relaxed lowercase min-w-0">{log.msg}</span>
                                        <span className="text-[10px] text-muted-foreground/50 italic shrink-0 whitespace-nowrap">{log.time}</span>
                                    </div>
                                ))}
                            </div>
                ...
                <div className="bg-gradient-to-br from-blue-600/20 via-transparent to-purple-600/20 border border-border/30 rounded-[3rem] p-10 lg:p-16 flex flex-col lg:flex-row items-center gap-12 relative overflow-hidden">
                    ...
                    <div className="lg:w-2/3 space-y-6 relative z-10">
                        <div className="flex items-center gap-3 px-4 py-2 bg-card/40 self-start rounded-full border border-border/50 w-fit">
                            <ShieldCheck className="w-4 h-4 text-emerald-500" />
                            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">Security Check Passed</span>
                        </div>
                        <h2 className="text-4xl font-black uppercase tracking-tighter text-foreground">Tier Management <span className="text-muted-foreground">Gatekeeper</span></h2>
                        <p className="text-sm text-muted-foreground font-bold uppercase leading-relaxed max-w-xl">Adjust features included in ORIGIN Free vs. Premium tiers dynamically. Changes propagate instantly to all active clusters without requiring a system reboot.</p>
                        <div className="flex items-center gap-6 pt-4">
                            <button className="px-10 py-4 bg-foreground text-background rounded-2xl font-black text-[12px] uppercase tracking-widest hover:scale-105 active:scale-95 transition-all">Configure Entities</button>
                            <button className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground hover:text-foreground transition-colors">Safety Audit Log</button>
                        </div>
                    </div>
