...
interface DailyTrackerProps {
    user: User;
    /** 'month' = single-month slider (dashboard); 'year' = last-12-months heatmap (profile) */
    view?: 'month' | 'year';
}

export default function DailyTracker({ user, view = 'month' }: DailyTrackerProps) {
    const [selectedDate, setSelectedDate] = useState(() => new Date());
    const [direction, setDirection] = useState(0); // -1: prev, 1: next
    const yearScrollRef = useRef<HTMLDivElement>(null);

    // In year view, scroll the heatmap to the end so the current month is visible.
    useEffect(() => {
        if (view !== 'year') return;
    ...
    // Last 12 months, for the yearly heatmap (profile view).
    const yearGroups = useMemo(() => {
        const today = new Date();
        const groups: Array<{ year: number; month: number; monthName: string; startPadding: number; days: Array<{ date: Date; level: number; count: number }> }> = [];
        let iter = new Date(today.getFullYear(), today.getMonth() - 11, 1);
        for (let i = 0; i < 12; i++) {
            const year = iter.getFullYear();
            const month = iter.getMonth();
            const monthName = iter.toLocaleDateString('en-US', { month: 'short' });
            ...
            groups.push({ year, month, monthName, startPadding, days });
            iter = new Date(year, month + 1, 1);
        }
        return groups;
    }, [dataMap]);
    ...
    const getLevelClass = (level: number) => {
        switch (level) {
            case 1: return 'bg-emerald-500/15 dark:bg-emerald-500/10 border border-emerald-500/40';
            case 2: return 'bg-emerald-500/35 dark:bg-emerald-500/20 border border-emerald-500/50';
            case 3: return 'bg-emerald-500/60 dark:bg-emerald-500/40 border border-emerald-500/60';
            case 4: return 'bg-emerald-500 dark:bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.35)] border border-emerald-700';
            case 0: return 'bg-slate-200/50 dark:bg-slate-800/40 border border-slate-400/60 dark:border-white/20';
            default: return 'bg-transparent border border-dashed border-slate-400/40 dark:border-white/15 opacity-50';
        }
    };

    const DAY_LABELS = ['S', 'M', 'T', 'W', 'T', 'F', 'S'];

    // ── Yearly heatmap (profile) — last 12 months, GitHub-style ──────────────
    if (view === 'year') {
        return (
            <Card className="neu-raised border border-border/60 overflow-hidden relative">
                    ...
                    <div className="flex items-center justify-between gap-3">
                        <div className="flex items-center gap-2">
                            ...
                            <div className="min-w-0">
                                ...
                                <p className="text-[10px] text-muted-foreground font-medium mt-0.5">Last 12 months</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-2">
                            {(user.streak || 0) > 0 && (
                ...
                <CardContent className="px-5 pb-4">
                    <div ref={yearScrollRef} className="overflow-x-auto pb-2 custom-scrollbar">
                        <TooltipProvider delayDuration={0}>
                            <div className="flex gap-3 min-w-max">
                                {yearGroups.map(group => {
                                    const now = new Date();
                                    const isCurrentMonth = group.year === now.getFullYear() && group.month === now.getMonth();
                                    return (
                                    <div key={`${group.year}-${group.month}`} className="space-y-1.5">
                                        <div className={cn(
                                            'text-[9px] font-bold uppercase tracking-wider ml-0.5 flex items-center gap-1',
                                            isCurrentMonth ? 'text-primary' : 'text-muted-foreground/60',
                                        )}>
                                            {group.monthName}
                                            {isCurrentMonth && <span className="w-1 h-1 rounded-full bg-primary" />}
                                        </div>
                                        <div className="grid grid-rows-7 grid-flow-col gap-[3px]">
                                            {Array.from({ length: group.startPadding }).map((_, i) => (
                                                <div key={`p-${i}`} className="w-[11px] h-[11px]" />
                                            ))}
                                                    ...
                                                    <Tooltip key={idx}>
                                                        <TooltipTrigger asChild>
                                                            <div className={cn(
                                                                ...
                                                                day.level !== -1 && 'hover:scale-125 hover:z-20',
                                                            )} />
                                                        </TooltipTrigger>
                    ...
                    <div className="flex items-center justify-end gap-1.5 mt-3">
                        ...
                        {[0, 1, 2, 3, 4].map(lvl => (
                            <div key={lvl} className={cn('w-2.5 h-2.5 rounded-[3px]', getLevelClass(lvl))} />
                        ))}
                        ...
                        <AnimatePresence initial={false} custom={direction} mode="wait">
                            ...
                            >
                                ...
                                {Array.from({ length: monthData.paddingEnd }).map((_, i) => (
                                    <div key={`p-end-${i}`} className="aspect-square rounded-[4px] bg-transparent border border-dashed border-slate-400/40 dark:border-white/15 opacity-50" />
                                ))}
                            </motion.div>
                        </AnimatePresence>
                    </div>
